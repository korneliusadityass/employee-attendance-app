...
class NeedConfirmCutiTahunanController extends GetxController with StateMixin<List<ApprovalCutiTahunanListModel>> {
  CutiTahunanApprovalService service;
  NeedConfirmCutiTahunanController(this.service);
  ...
  //refuse
  Future cutiTahunanRefuse(int id) async {
    try {
      final result = await service.cutiTahunanRefuse(id);
      result.fold((failure) {
        Get.dialog(Dialog(
        ...
        Get.offNamed(Routes.pageApprovalCutiTahunan);
        Get.find<CutiTahunanApprovalController>().execute();
        Get.dialog(Dialog(
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(8),
          ),
...
// Approve
  Future cutiTahunanApproved(int id) async {
    try {
      final result = await service.cutiTahunanApproved(id);
      result.fold((failure) {
        Get.dialog(Dialog(
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(10),
