...
class LaporanIzinSakit extends GetView<LaporanIzinSakitController> {
  LaporanIzinSakit({super.key});
...
// class _LaporanIzinSakitState extends State<LaporanIzinSakit> {
  ...
  // final controller = Get.find<LaporanIzinSakitController>();
  final exportLaporanIzinSakitController =
      Get.find<ExportLaporanIzinSakitController>();
  DateTime? endDate;
  DateTime? startDate;
  ...
  // AREA TABLE CONTOH
  List<dynamic> dataIzinKhusus = [];
  List<dynamic> data = [];

  Future<void> fetchData() async {
    print(args);
    controller.dataLaporanIzinSakit.value = await controller.execute(
      args,
      page: page,
  ...
  Widget build(BuildContext context) {
    void downloadExcel() async {
      ...
      final hasil = await exportLaporanIzinSakitController.exportLaporan(
        args,
      ...
      List<ListLaporanIzinSakitModel> data = hasil;
      ...
      final file = File('${downloadDirectory.path}/LaporanIzinSakit.xlsx');
                      ...
                      child: DataTable(
                        ...
                        rows: controller.isFetchData.value
                            ...
                            : List<DataRow>.generate(
                                controller.dataLaporanIzinSakit.length,
                                (int index) {
                                  // print('TIPE: ${dataIzin3Jam[index]['waktu_awal'].runtimeType}');
                                  int rowIndex =
                                      (page - 1) * pageSize + index + 1;
                                  ListLaporanIzinSakitModel data =
                                      controller.dataLaporanIzinSakit[index];
                                  return DataRow(
                                    cells: [
                    ...
                    () => Row(
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed:
                              controller.dataLaporanIzinSakit.length >= pageSize
                                  ? () => onPageChange(page + 1)
                                  : null,
                        ),
                      ],
